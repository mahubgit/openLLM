...
remove_model() {
    ...
    if check_model_exists "$model_name"; then
        ...
        if is_model_running "$model_name"; then
            log_message "ERROR" "Model '$model_name' is currently running"
            log_message "INFO" "Please stop the model first with './openllm.sh stop $model_name'"
            exit 1
        fi
        
        # Remove the model
        rm -rf "$MODELS_DIR/$model_name"
        log_message "SUCCESS" "Model '$model_name' removed successfully"
    else
        exit 1
    fi
}
